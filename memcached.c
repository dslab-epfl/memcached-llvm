...
#ifdef CLOUD9_TEST
static int server_main (int argc, char **argv, int notiffd) {
#else
...
#ifdef CLOUD9_TEST
    // Send a notification by closing the file descriptor
    close(notiffd);
#endif
...
static pid_t start_server(in_port_t *port_out, bool daemon, int timeout) {
#if 0 // CLOUD9: Don't need to do this for Cloud9
    char environment[80];
    ...
    */
    ...
    // CLOUD9: This is the synchronization object to ensure the server is initialized
...
#if 0 // CLOUD9: No need to make it a daemon
        if (!daemon) {
            argv[arg++] = "./timedrun";
            argv[arg++] = tmo;
        }
...
// CLOUD9: We replaced here the original synchronization code with our own
        close(fdpair[0]);
        int ret = server_main(arg-1, argv, fdpair[1]);
        printf("done!\n");
        return ret;
    ...
    close(fdpair[1]);
    int data;
    res = read(fdpair[0], &data, sizeof(data));
    assert(res == 0);
    close(fdpair[0]);
...
__attribute__((unused)) static enum test_return test_issue_44(void) { // CLOUD9: Test case not used
    in_port_t port;
...
__attribute__((unused)) static enum test_return stop_memcached_server(void) {
    close(sock);
...
__attribute__((unused)) static enum test_return test_binary_flush(void) { // CLOUD9: Test case not used
    return test_binary_flush_impl("test_binary_flush",
                                  PROTOCOL_BINARY_CMD_FLUSH);
}

__attribute__((unused)) static enum test_return test_binary_flushq(void) { // CLOUD9: Test case not used
...
static void send_symbolic_packet(void) {
  union {
      protocol_binary_request_no_extras request;
      protocol_binary_response_no_extras response;
      char bytes[64];
  ...
  klee_make_symbolic(send.bytes, sizeof(send.bytes), "sympacket");
  send.bytes[0] = '\x80'; // Force a binary packet

  safe_send(send.bytes, len, false);
  safe_recv_packet(receive.bytes, sizeof(receive.bytes));
}

__attribute__((unused)) static enum test_return test_symbolic_packet(void) {
  send_symbolic_packet();
  return TEST_PASS; // If you can make it here, it's a pass
}

static void send_symbolic_binary_cmd(void) {
  union {
      protocol_binary_request_no_extras request;
      protocol_binary_response_no_extras response;
      char bytes[64];
  ...
  klee_make_symbolic(key, sizeof(key), "key"); key[3] = 0;
  klee_make_symbolic(value, sizeof(value), "value"); value[3] = 0;
  klee_make_symbolic(&cmd, sizeof(cmd), "cmd");

  klee_make_symbolic(send.bytes, sizeof(send.bytes), "sympacket");

  //klee_print_expr("key", *((uint32_t*)key));
  //klee_print_expr("value", *((uint32_t*)value));
...
__attribute__((unused)) static enum test_return test_symbolic_binary_cmd(void) {
  send_symbolic_binary_cmd();

  return TEST_PASS; // If you can make it here, it's a pass
}

static enum test_return test_2symbolic_binary_cmds(void) {
  send_symbolic_binary_cmd();
  send_symbolic_binary_cmd();
...
__attribute__((unused)) static enum test_return test_2symbolic_cmds(void) {
  send_symbolic_packet();
  send_symbolic_packet();
...
__attribute__((unused)) static enum test_return test_issue_101(void) { // CLOUD9: Test-case not invoked
    const int max = 2;
    enum test_return ret = TEST_PASS;
    int fds[2];
    int ii = 0;
...
struct testcase symbolic_testcases[] = {
    { "start_server", start_memcached_server },
    { "symbolic_2bin", test_2symbolic_binary_cmds },
    //{ "symbolic_2any", test_2symbolic_cmds },
    {NULL, NULL}
};

struct testcase concrete_testcases[] = {
    ...
    //{ "issue_44", test_issue_44 }, CLOUD9: We don't run this because kill() is not yet supported
    { "vperror", test_vperror },
    //{ "issue_101", test_issue_101 }, CLOUD9: We don't run this because kill() is not yet supported
    ...
    //{ "binary_flush", test_binary_flush }, CLOUD9: We don't run this because flushing doesn't happen since the time-outs aren't for real
    //{ "binary_flushq", test_binary_flushq }, CLOUD9: We don't run this because flushing doesn't happen since the time-outs aren't for real
    ...
    //{ "stop_server", stop_memcached_server }, CLOUD9: We don't run this because kill() is not yet supported
    { NULL, NULL }
};
...
int main(int argc, char **argv)
{
    ...
    struct testcase *testcases = concrete_testcases;

    if (argc > 0 && strcmp(argv[0], "--sym") == 0) {
      printf("Using symbolic test cases!\n");
      testcases = symbolic_testcases;
    }
